...
async def fetch_ranked(brawlhalla_id: str) -> dict | None:
    ...
    try:
        # Run the blocking GET in a thread to keep the event loop responsive
        ...
        # Most‑played legend this season (by games)
        top_legend_id = None
        if data.get("legends"):
...
async def build_leaderboard(players: list[dict]) -> list[dict]:
    ...
    for player, stats in zip(players, stats_list):
        if stats is None:
            stats = {}  # fallback to defaults below
        enriched.append({
            **player,
            "display_name":  player["name"],          # always use the provided name
...
def generate_image(leaderboard: list[dict], output_path: str = "leaderboard.png", start_rank: int = 1):
    ...
    for i, p in enumerate(leaderboard):
        ...
        # ── Tier frame overlay on portrait ──────────────────────────────────
